CREATE OR REPLACE PACKAGE Z_FECHAMENTO_CONTABIL_CPROC IS
  ...
  CURSOR rHeader(pCabecalho varchar2) IS
    SELECT REGEXP_SUBSTR(pCabecalho, '[^,]+', 1, LEVEL) AS campo
      FROM dual
    CONNECT BY LEVEL <= regexp_count(pCabecalho, ',') + 1
  ...
  CURSOR Saldo(pEmpresa          varchar2,
    ...
    SELECT 'X80' AS tabela,
      ...
      JOIN x2002_plano_contas x2002
        ON x2002.ident_conta = x80.ident_conta
      JOIN x2003_centro_custo x2003
        ON x2003.ident_custo = x80.ident_custo
    ...
    SELECT 'X02' AS tabela,
      ...
      FROM x02_saldos x02
      JOIN x2002_plano_contas x2002
        ON x2002.ident_conta = x02.ident_conta
     WHERE x02.cod_empresa = pEmpresa
       ...
       AND (pTipoEncerramento = '1' OR NOT EXISTS
            (SELECT 1
  ...
  CURSOR Fechamento(pEmpresa varchar2, pEstab varchar2, pCompetencia date) IS
    SELECT retorno.cod_empresa,
      ...
      FROM (SELECT cod_empresa,
                   ...
                   (CASE
                     WHEN x01_contabil.ind_deb_cre = 'C' THEN
                      SUM(x01_contabil.vlr_lancto)
                     ELSE
                      0
                   END) AS valor_credito,
                   (CASE
                     WHEN x01_contabil.ind_deb_cre = 'D' THEN
                      SUM(x01_contabil.vlr_lancto)
                     ELSE
                      0
                   END) AS valor_debito
             ...
             WHERE x01_contabil.cod_empresa = pEmpresa
               ...
               AND (x01_contabil.data_lancto BETWEEN
                   TO_DATE(pCompetencia, 'DD/MM/RRRR') AND
  ...
  CURSOR Saldo_Novo_X80(pEmpresa     varchar2,
    ...
    SELECT retorno.cod_empresa,
      ...
      FROM (SELECT cod_empresa,
                   ...
                   END) AS valor_debito
              FROM x01_contabil
              JOIN x2002_plano_contas
                ON x2002_plano_contas.ident_conta = x01_contabil.ident_conta
              JOIN x2003_centro_custo
                ON x2003_centro_custo.ident_custo = x01_contabil.ident_custo
             WHERE x01_contabil.cod_empresa = pEmpresa
               AND x01_contabil.cod_estab = pEstab
               AND (x01_contabil.data_lancto BETWEEN
                   TO_DATE(pCompetencia, 'DD/MM/RRRR') AND
